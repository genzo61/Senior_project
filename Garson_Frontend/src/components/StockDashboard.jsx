...
function StockDashboard() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const res = await fetch(`http://${window.location.hostname}:8081/api/products`);
  ...
  const confirmStockUpdate = async (id) => {
    ...
    try {
      const res = await fetch(`http://${window.location.hostname}:8081/api/products/${id}/stock?quantity=${newStock}`, {
        method: 'PUT'
      });
      if (res.ok) {

      ...
      <div className="absolute bottom-0 left-0 right-0 max-h-[calc(100dvh-0.5rem-env(safe-area-inset-top))] overflow-y-auto overscroll-y-contain border-t border-cyan-300/25 bg-slate-900 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] shadow-2xl md:bottom-6 md:left-auto md:right-6 md:top-[max(1rem,env(safe-area-inset-top))] md:w-[440px] md:rounded-3xl md:border md:p-5 md:pb-5">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button
            type="button"
            onClick={onClose}
            className="min-h-10 rounded-full border border-slate-700 px-3 py-1 text-sm text-slate-300 transition hover:border-cyan-300/60"
          >
            Kapat
          </button>
        </div>

        <div className="mb-4 max-h-[min(44dvh,22rem)] space-y-3 overflow-y-auto pr-1 md:max-h-[52dvh]">
          {cartItems.length === 0 ? (
              ...
              <div key={line.lineId} className="rounded-xl border border-slate-800 bg-slate-950/90 p-3">
                ...
                <div className="mb-2 flex items-center gap-2">
                  <button
                    ...
                    className="h-10 w-10 rounded-lg border border-slate-700 text-base text-slate-200 sm:text-sm"
                  >
                    -
                  </button>
                ...
                <label className="block text-xs text-slate-400">
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2.5 text-base text-slate-100 outline-none focus:border-cyan-300/70 sm:text-sm"
                  />
                </label>
              </div>
            ))
        ...
        <label className="mb-3 block text-xs text-slate-400">
          ...
          <textarea
            ...
            className="mt-1 w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2.5 text-base text-slate-100 outline-none focus:border-cyan-300/70 sm:text-sm"
          />
        </label>
        ...
        <div className="flex flex-col gap-3 border-t border-slate-800 pt-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            onClick={onCheckout}
            className={`min-h-11 w-full rounded-xl px-5 py-3 text-sm font-bold transition sm:w-auto ${
